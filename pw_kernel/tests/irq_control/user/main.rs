...
fn test_interrupt_flow() -> Result<()> {
    ...
    match result {
        Ok(wait_ret) => {
            let sigs = wait_ret.pending_signals;
...
fn test_backward_compat_ack() -> Result<()> {
    ...
    syscall::interrupt_ack(handle::TEST_INTERRUPTS, sigs.pending_signals)?;
...
fn test_clear_pending_only() -> Result<()> {
    ...
    // CLEAR_PENDING without ENABLE implicitly disables the interrupt.
    // This is by design: the ENABLE flag's absence means "disable".
    let status = syscall::interrupt_status(handle::TEST_INTERRUPTS, signals::TEST_IRQ)?;
    if status.contains(InterruptStatus::ENABLED) {
        pw_log::error!("CLEAR_PENDING without ENABLE should disable the interrupt");
        return Err(Error::FailedPrecondition);
    }
